...
// -------
//
function ciniki_events_wng_eventLoad($ciniki, $tnid, $request, $permalink) {
    ...
    $strsql = "SELECT ciniki_events.id, "
        ...
        . "ciniki_events.long_description AS description, "
    ...
    $rc = ciniki_core_dbHashQueryIDTree($ciniki, $strsql, 'ciniki.artclub', array(
        array('container'=>'events', 'fname'=>'id', 
            ...
            'url', 'synopsis', 'description', 'object', 'object_id',
            'ticketmap1_image_id', 'ticketmap1_ptext', 'ticketmap1_btext', 'ticketmap1_ntext', 
            )),
        array('container'=>'images', 'fname'=>'image_permalink', 
            'fields'=>array('image-id'=>'image_id', 'title'=>'image_name', 'permalink'=>'image_permalink',
    ...
    if( isset($request['session']['customer']['price_flags']) ) {
        ...
        if( isset($request['session']['customer']['membership_expiration']) && ($price_flags&0x20) == 0x20 ) {
            ...
            //
            if( $event['start_date_ts'] > $request['session']['customer']['membership_expiration'] ) {
    ...
    $strsql = "SELECT id, name, url, description "
        ...
        . "WHERE tnid = '" . ciniki_core_dbQuote($ciniki, $tnid) . "' "
        . "AND event_id = '" . ciniki_core_dbQuote($ciniki, $event['id']) . "' "
        . "";
    $rc = ciniki_core_dbHashQueryIDTree($ciniki, $strsql, 'ciniki.events', array(
    ...
    //
    if( ciniki_core_checkModuleFlags($ciniki, 'ciniki.sponsors', 0x02) ) {
        ciniki_core_loadMethod($ciniki, 'ciniki', 'sponsors', 'wng', 'sponsorRefList');
        $rc = ciniki_sponsors_wng_sponsorRefList($ciniki, $tnid, $request, 'ciniki.events.event', $event['id']);
        if( $rc['stat'] != 'ok' ) {
            return $rc;
        }
    ...
    // FIXME: Only linked to artcatalog and currently artcatalog not setup for WNG
    //
