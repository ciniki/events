...
function ciniki_events_fileGet($ciniki) {
    ...
    $rc = ciniki_events_checkAccess($ciniki, $args['tnid'], 'ciniki.events.fileGet', 0); 
    if( $rc['stat'] != 'ok' ) { 
        return $rc;
    }   
    ...
    $strsql = "SELECT ciniki_event_files.id, "
        ...
        . "WHERE ciniki_event_files.tnid = '" . ciniki_core_dbQuote($ciniki, $args['tnid']) . "' "
    ...
    if( !isset($rc['file']) ) {
        return array('stat'=>'fail', 'err'=>array('code'=>'ciniki.events.21', 'msg'=>'Unable to find file'));
    }
