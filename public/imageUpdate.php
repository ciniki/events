...
function ciniki_events_imageUpdate(&$ciniki) {
    ...
    $rc = ciniki_events_checkAccess($ciniki, $args['tnid'], 'ciniki.events.imageUpdate'); 
    if( $rc['stat'] != 'ok' ) { 
        return $rc;
    }
    ...
    if( !isset($rc['item']) ) {
        return array('stat'=>'fail', 'err'=>array('code'=>'ciniki.events.29', 'msg'=>'Event image not found'));
    }
    $item = $rc['item'];
    ...
    if( isset($args['name']) ) {
        ...
        if( $rc['num_rows'] > 0 ) {
            return array('stat'=>'fail', 'err'=>array('code'=>'ciniki.events.30', 'msg'=>'You already have an image with this name, please choose another name'));
        }
    }
    ...
    return ciniki_core_objectUpdate($ciniki, $args['tnid'], 'ciniki.events.image', $args['event_image_id'], $args);
}
?>
